...
(function (root, factory) {
    ...
    describe('ApiKey', function () {
        it('should create a known ApiKey', () => {
            let apiKey = new citypay_api_client.ApiKey('PC2', '7G79TG62BAJTK669');
            var key = apiKey.generateApiKey("acb875aef083de292299bd69fcdeb5c5",
                new Date(2020, 0, 1, 9, 23, 12, 0)
            )
            expect(key).to.be("UEMyOkFDQjg3NUFFRjA4M0RFMjkyMjk5QkQ2OUZDREVCNUM1Ol6Q3tCMPsYvqNhFelRD2zQHYlZquBGCY/6ZpZ0AngTF");
        });


        for (let i = 0; i < data.length; i++) {
            it('should generate and match example data, row: ' + i, () => {
                const row = data[i];
                const key = new citypay_api_client.ApiKey(row[0], row[1]);
                expect(row[4]).to.be(key.generateApiKey(row[2], new Date(row[3] + "Z")));
            })
        }
